  ...
  try {
    for (const pal of ["a", "c"]) {
      await conFixture(`peluqueria-paleta-${pal}`, async (url) => {
        ...
        const v = await p.evaluate(() => { const v = document.querySelector<HTMLVideoElement>("#hero video")!; return { src: v.currentSrc.replace(/^.*\/media\//, ""), paused: v.paused, t: v.currentTime, w: v.videoWidth, h: v.videoHeight }; }) as { err?: string; src?: string; paused?: boolean; t?: number; w?: number; h?: number };
