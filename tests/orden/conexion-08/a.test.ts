...
import {
  ...
  conTemporalAsync, correrLargo, fixture, get, git, medirPng, nombresTap, rutaStorage, tokensDeScript,
} from "./_comun.ts";
...
/** GET con `Range: bytes=0-0` y tope de 10 s: { status, tipo }. */
async function cabeza(url: string): Promise<{ status: number; tipo: string }> {
  const corte = new AbortController();
  const reloj = setTimeout(() => corte.abort(), 10000);
  try {
    const r = await fetch(url, { headers: { Range: "bytes=0-0" }, signal: corte.signal });
...
test("dev-fixtures/peluqueria-paleta-a.json y -c.json tienen `contact.phone` = `+972 3-000-0000` y `brand.logo` / `brand.logoDark` como urls de Storage bajo `clients/test-b4-peluqueria-<p>/media/branding/logo.png` y `logo-dark.png` (https, un GET con `Range: bytes=0-0` responde 200/206 e `image/png`); nada más cambia en los fixtures (`git diff 637ed2b HEAD -- dev-fixtures/*.json` = esas tres claves en cada uno)", async () => {
  const urls: string[] = [];
  ...
  // (3) Las cuatro urls responden de verdad, con el tipo que Storage debe servir.
  for (const url of urls) {
    const r = await cabeza(url);
    assert.ok(r.status === 200 || r.status === 206, `GET con Range a ${url.slice(0, 110)}… responde 200 o 206 (respondió ${r.status})`);
    assert.match(r.tipo, /^image\/png\b/, `…y con content-type image/png (respondió «${r.tipo}»)`);
  }
  // (4) Nada más cambia: contra el commit aprobado de CONEXION-07, las únicas hojas distintas son las tres de esta orden.
  for (const p of ["a", "c"] as const) {
    ...
    assert.deepEqual(cambios(antes, ahora), [...CLAVES_NUEVAS].sort(), `${rel}: sólo cambian las tres claves de esta orden respecto de ${CONEXION_07.aprobado.T}`);
  }
});
