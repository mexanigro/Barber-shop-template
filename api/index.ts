...
import { resolveServiceMetadata } from "../src/lib/api/service-metadata.js";
import {
  ADMIN_ROLES,
  canAssignRole,
  canRemoveRole,
...
${toolsFragment}`;
  ...
  // Public booking goes through server-side Admin SDK writes; Firestore rules
  // intentionally reject unauthenticated appointment/manifest creation.
  app.post("/api/book", async (req, res) => {
    try {
      ...
      try {
        ...
        tenantConfig = configSnap.exists ? (configSnap.data() ?? {}) : {};
      } catch (err) {
        console.warn("[Book] tenant config read failed:", err instanceof Error ? err.message : err);
      }
      const serviceMetadata = resolveServiceMetadata(tenantConfig, serviceId);
  ...
  app.post("/api/notify-booking", async (req, res) => {
    try {
      ...
      console.error("[Notify Booking] failed:", error);
