 ...
 *
 * Uso: node tools/material/transicion.mjs <fixture> [--franja 0.10] [--json]
...
const frames = [1, dur / 2].map((t, i) => {
  const out = path.join(tmp, `pie-${i}.png`);
  const r = spawnSync("ffmpeg", ["-v", "error", "-y", "-ss", String(t), "-i", local(clip), "-frames:v", "1", "-vf", `crop=iw:ih*${franja}:0:ih*${1 - franja}`, "-update", "1", out]);
  if (r.status !== 0) { console.error(String(r.stderr)); process.exit(1); }
  return { role: `pie t=${t.toFixed(1)}s`, src: out, kind: "image" };
});
const { rows } = await medir([...frames, { role: "foto del local", src: local(foto), kind: "image" }], paletaDe(fx));
...
const out = { fixture: fx, franja, pie: { frames: pie, mean: pieM }, local: fot, dH, dL, neutro, relation, mechanism };
...
console.log(`  ΔH ${dH}°${neutro ? " (neutro: tono no juzgado)" : ""} · ΔL ${dL} → relación: ${relation ?? "NINGUNA (fuera de los tres casos)"} · mecanismo propuesto: ${mechanism}`);
