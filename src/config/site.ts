...
import { mergeWeeklyHours, type WeeklyHours } from "../lib/hours-merge";
...
function applyHoursOverride(override: DeepPartial<SiteConfig>): void {
  const hours = override.hours;
  if (!hours || typeof hours !== "object" || Array.isArray(hours)) return;
  (siteConfig as Record<string, unknown>).hours = mergeWeeklyHours(
    siteConfig.hours as WeeklyHours,
    hours,
  );
}
...
export function switchSiteLanguage(lang: UiLanguage): void {
  ...
  if (_tenantOverride) {
    ...
    applyHoursOverride(_tenantOverride);
  }
  _applyBusinessMode();
  _applyNicheFeatures();
  _applyVisibleServicesFilter();
