...
import { mergeTenantWeeklyHours, type PartialWeeklyHours } from "../lib/hours-merge";
...
function applyTenantHours(override: DeepPartial<SiteConfig>): void {
  const hours = override.hours;
  if (!hours || typeof hours !== "object" || Array.isArray(hours)) return;
  (siteConfig as Record<string, unknown>).hours = mergeTenantWeeklyHours(
    siteConfig.hours as PartialWeeklyHours | undefined,
    hours as PartialWeeklyHours,
  );
}
...
export function switchSiteLanguage(lang: UiLanguage): void {
  ...
  if (_tenantOverride) {
    ...
    applyTenantHours(_tenantOverride);
  }
  _applyBusinessMode();
  _applyNicheFeatures();
  _applyVisibleServicesFilter();
