...
export function WhyChooseUs({
  ...
  /* ── Estetica: editorial split — large image left, benefits right ───── */
  if (isEstetica) {
    return (
      <section id="why-choose-us" className="flex flex-col justify-center bg-background px-5 py-12 transition-colors duration-300 sm:px-6 sm:py-28 lg:block">
        <div className="mx-auto max-w-7xl">
          <div className="grid grid-cols-1 items-center gap-10 sm:gap-16 lg:grid-cols-2 lg:gap-24">
            ...
            <div className="relative">
              <motion.div
                {...nicheScaleIn(niche)}
                className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-border shadow-elevated"
              >
                <img
              ...
              <motion.div
                ...
                className="absolute -bottom-6 -end-6 hidden w-52 overflow-hidden rounded-3xl bg-accent-light p-7 shadow-xl shadow-accent/30 hover:rotate-0 md:block [transition:transform_0.5s_cubic-bezier(0.23,1,0.32,1),box-shadow_0.4s_cubic-bezier(0.23,1,0.32,1)] lg:hover:shadow-[0_24px_60px_-12px_rgba(0,0,0,0.25)]"
              >
                <Star className="mb-3 text-primary-foreground" size={32} fill="currentColor" />
            ...
            <div>
              ...
              <motion.h2
                ...
                viewport={VIEWPORT_ONCE}
                transition={{ duration: NICHE_DURATION[flavor], ease: NICHE_EASING[flavor] }}
                className="mb-8 font-serif text-3xl font-normal tracking-wide text-card-foreground sm:mb-14 sm:text-4xl md:text-5xl"
              >
                {resolvedSubtitle}
              </motion.h2>
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                {sectionConfig.benefits.map((benefit, i) => {
                  ...
                  return (
                    <motion.div
                      key={`benefit-estetica-${benefit.title.slice(0, 20)}-${i}`}
                      initial={{ opacity: 0, x: i % 2 === 0 ? -X_IN : X_IN }}
                      ...
                      className="group rounded-2xl border border-border bg-background p-6 hover:border-accent/30 dark:bg-background/50 [transition:border-color_0.3s_cubic-bezier(0.23,1,0.32,1),background-color_0.3s_ease]"
                    >
