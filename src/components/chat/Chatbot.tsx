...
// Official WhatsApp logo (single-path SVG, currentColor for fill).
...
// Employment niche replaces the AI chatbot with a direct WhatsApp link.
// Same floating position contract as the chat button so it sits above the
// hero CTAs in-view and tucks above the audience bar on scroll.
function WhatsAppFloatingButton() {
  ...
  const href = `https://wa.me/${cleaned.replace(/^\+/, "")}`;
  ...
  return (
    <motion.a
      ...
      style={{
        transition:
          "bottom 0.5s cubic-bezier(0.23,1,0.32,1), background-color 0.2s cubic-bezier(0.23,1,0.32,1), box-shadow 0.3s cubic-bezier(0.23,1,0.32,1)",
      }}
    >
        ...
        {!isOpen && (
          <motion.button
            ...
            className={cn(
              ...
              isInHero ? (isEstetica ? "bottom-[calc(33%-25px)]" : isEmployment ? "bottom-[35%]" : "bottom-[10.5%]") : "bottom-20 sm:bottom-[5.5rem]",
            )}
            style={{
